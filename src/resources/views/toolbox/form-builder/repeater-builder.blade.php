                                        ...
                                        <tr class="align-top">
                                            ...
                                            @foreach($repeaterColumns as $column)
                                                ...
                                                <td
                                                    ...
                                                    @change="$wire.updateRepeaterDefaultRowValue({{ $rowIndex }}, '{{ $columnName }}', $event.target.type === 'checkbox' ? $event.target.checked : ($event.target.multiple ? Array.from($event.target.options).filter(o => o.selected).map(o => o.value) : $event.target.value))"
                                                >
                                                    @if($subField)
                ...
                @foreach($repeaterColumns as $columnIndex => $column)
                    ...
                    <div wire:key="{{ $columnEditorKey }}" class="rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <div
                                @change="$wire.updateRepeaterDefaultRowValue({{ $activeRepeaterRow }}, '{{ $columnName }}', $event.target.type === 'checkbox' ? $event.target.checked : ($event.target.multiple ? Array.from($event.target.options).filter(o => o.selected).map(o => o.value) : $event.target.value))"
                            >
                                @if($subField)
                                    {{ $subField->render(false, false) }}
                                @endif
