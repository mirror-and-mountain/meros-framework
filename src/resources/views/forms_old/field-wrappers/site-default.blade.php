...
<div
    ...
    class="meros-field nice-form-group{{ $hasFieldError ? ' invalid' : '' }} {{ $field->handle === 'password' ? 'meros-password-field' : '' }}" {{ $label === false ? 'style=margin-top:0;' : '' }}
>
    @if ($label !== false)
        @if (!$isFieldSet)
            <label 
    ...
    @include($view, ['attributes' => $attributesString])
