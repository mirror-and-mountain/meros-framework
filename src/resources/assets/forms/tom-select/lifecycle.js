...
export function cacheSelectValue(select, value) {
    ...
    if (Array.isArray(value)) {
        tomSelectValueCache.set(cacheKey, value.map((item) => String(item)));
        return;
    }

    if (value === null || value === undefined) {
        tomSelectValueCache.set(cacheKey, null);
        return;
    }

    tomSelectValueCache.set(cacheKey, String(value));
}
...
export function syncTomSelectValue(select) {
    try {
        ...
        if (isMultiple) {
            ...
            if (selectedValue !== null && selectedValue !== undefined && select.tomselect.setValue) {
