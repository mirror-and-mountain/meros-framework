...
function applyTomSelectRepeaterColumnSync(detail) {
    ...
    const options = Array.isArray(detail.options) ? detail.options : [];
    const repeaterLocationKey = detail.repeaterLocationKey ? String(detail.repeaterLocationKey) : null;
    const repeaterTables = repeaterLocationKey
        ? document.querySelectorAll(`.meros-repeater-table[data-repeater-location-key="${repeaterLocationKey}"]`)
        : document.querySelectorAll('.meros-repeater-table');
...
function updateRepeaterSelectValue(select, value) {
    ...
    if (select.tomselect && typeof select.tomselect.setValue === 'function') {
        if (isMultiple) {
            ...
            select.tomselect.setValue(value === null || value === undefined ? '' : String(value), true);
        }
...
function applyTomSelectRepeaterRowValueSync(detail) {
    if (!detail || !detail.fieldName || detail.columnIndex === undefined || detail.rowIndex === undefined) {
        return;
    }

    const value = detail.value;
    const columnIndex = Number(detail.columnIndex);
    const targetRowIndex = Number(detail.rowIndex);
    const repeaterLocationKey = detail.repeaterLocationKey ? String(detail.repeaterLocationKey) : null;

    if (Number.isNaN(columnIndex) || Number.isNaN(targetRowIndex)) {
        return;
    }
    ...
    repeaterTables.forEach((table) => {
        const row = table.querySelector(`tbody tr[data-repeater-row-index="${targetRowIndex}"]`);
